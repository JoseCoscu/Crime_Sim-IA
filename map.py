...
fire_fighters = []
id = 1
ciudadanos = 20  # nueva line por habitantes
empleados = 10  # nueva line por habitantes
criminales = 10
bomberos = 5
oficiales = 10
indice_agresividad = 'Bajo'
indice_crim = 'Bajo'
...
def citizens_threads(i):
    global data
    transition, data = citizens[i](100)
    if transition:
        citizens[i] = Criminal(id, 'Criminal_' + 'Neewww', r.choice(all_locations), [], [], time_meter, G,
                               all_locations,
                               r.choice(houses), indice_crim, indice_agresividad, 1)

        criminals.append(citizens[i])
        citizens.remove(citizens[i])
        criminals[-1](100)
    else:
        citizens[i] = Officer(id, 'Officer_' + 'Neewww', station, [], [], 10, time_meter, G, all_locations,
                              r.choice(houses),
                              station)

        officers.append(citizens[i])
        citizens.remove(citizens[i])
        officers[-1](100)
...
# arreglar datos
if indice_agresividad == 0.8:
    indice_agresividad = 0.2
elif indice_agresividad == 0.2:
    indice_agresividad = 0.8

if indice_crim == 0.8:
    indice_crim = 0.2
elif indice_crim == 0.2:
    indice_crim = 0.8

    # add_data(ciudadanos, empleados, criminales, bomberos, oficiales, indice_agresividad, indice_crim, cant_robs,
    #          cant_heridos, cant_incendios,cant_apresados, cant_rob_report,0)
    # con+=1
